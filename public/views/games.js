...
export async function gamesView(root) {
  ...
  const visibleRows = () => [...games]
    ...
    .filter(matchesQuery)
    .sort(cmp);
  // Rows on screen now: the current page at the chosen size.
  const shownRows = () => pg.slice(visibleRows());
  ...
  // Checkbox selection (change, so it does not open the game row); the pager's
  // rows-per-page select fires here too.
  list.addEventListener('change', e => {
    if (pg.handle(e.target)) return render();
  ...
  list.addEventListener('click', async e => {
    ...
    if (sh) {
      ...
      pg.reset(); // reordering changes what each page holds; start from the top
      return render();
    }
    const fbtn = e.target.closest('button[data-filter]');
    if (fbtn) { filter = fbtn.dataset.filter; pg.reset(); return render(); }
  ...
  root.querySelector('#game-search').addEventListener('input', e => { query = e.target.value.trim().toLowerCase(); pg.reset(); render(); });
