...
export async function logView(app) {
  ...
  const body = events.length
    ? `<table class="log-table"><thead><tr><th>When</th><th>Who</th><th>Where</th><th>Action</th><th>Detail</th></tr></thead><tbody>${events.map(row).join('')}</tbody></table>`
    : '<p class="empty">No activity recorded yet.</p>';
  app.innerHTML = `<h1>Activity log</h1>
    <p class="muted">Sign-ins and material actions, newest first (showing ${events.length}).</p>
    ${body}`;
}
