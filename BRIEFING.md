...
3. Per-drill ease (SM-2/FSRS-lite) fitted from the stored `reviews[]` history, replacing the fixed ladder; reviews now record answer time (`ms`) and prior ladder position, and mirrors sync between machines, so the data is accumulating.
...
6. Hosted build: `public/` unchanged, `api.js` swapped for a browser backend (Stockfish WASM worker plus Anthropic API client with a user-pasted key). Ask before adding anything that uses an API key. The opening eval cache is the natural seed data for the WASM worker.
