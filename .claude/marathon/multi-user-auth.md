...
- [ ] Phase 2b: Per-user DRILLS + PATTERNS stores. Drills carry per-user review state (ladder/history) so they need per-user stores: data/users/<id>/drills.json + mirror drills-<host>.json, Supabase key drills:<userId>; syncAllDrills(userId), dueDrills/reviewDrill/etc. scoped; getPatternNotes/savePatternNotes(userId). Migrate data/drills.json -> kai. Update drills.test.js (it readdirs DATA_DIR for the mirror file). This is the piece deferred out of Phase 2.
