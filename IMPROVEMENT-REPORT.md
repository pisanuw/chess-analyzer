...
Also worth measuring before deciding: dropping the duplicated `san/uci/fen*` from `analysis.moves[]` (keep them only in `game.moves[]` and join by ply) would roughly halve file sizes and the data repo, at the cost of a one-time migration and touching every reader of `analysis.moves`.
